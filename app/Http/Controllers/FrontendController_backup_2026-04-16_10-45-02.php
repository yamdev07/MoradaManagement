...
class FrontendController extends Controller
{
    ...
    // Réservation multitenant
    public function multitenantReservation()
    {
        $currentHotel = app('current_hotel');
        if (!$currentHotel) {
            return redirect('/')->with('error', 'Aucun hôtel sélectionné');
        ...
        // Récupérer les types de chambres pour ce tenant
        $roomTypes = \App\Models\Type::whereHas('rooms', function ($q) use ($currentHotel) {
            $q->where('tenant_id', $currentHotel->id);
        })->get();
        
        return view('frontend.multitenant.reservation', compact('currentHotel', 'roomTypes'));
    }

    // API pour obtenir les chambres disponibles (optimisée)
    public function availableRooms(Request $request)
    {
        try {
            ...
            // Récupérer l'hôtel actuel depuis la vue partagée ou session
            $currentHotel = view()->shared('currentHotel');
            ...
            // Si toujours pas d'hôtel, essayer de récupérer depuis l'utilisateur connecté
            if (!$currentHotel && auth()->check() && auth()->user()->tenant_id) {
                $currentHotel = \App\Models\Tenant::find(auth()->user()->tenant_id);
            }
            ...
            // Chambres réservées pour ce tenant (optimisé)
            $bookedRoomIds = Transaction::whereIn('status', ['reservation', 'active'])
                ->where('tenant_id', $currentHotel->id)
                ->where(function ($q) use ($checkIn, $checkOut) {
                    $q->where('check_in', '<', $checkOut)
            ...
            // Chambres disponibles pour ce tenant (optimisé)
            ...
            return response()->json([
                'rooms' => [],
                'count' => 0,
                'error' => 'Erreur lors de la recherche des chambres: ' . $e->getMessage()
            ], 500);
        }
    }
}