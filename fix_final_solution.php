...
// Solution finale pour corriger le problème de customer (avec email unique)

echo "🔧 Solution finale pour le problème de customer de la transaction ID 13\n\n";
...
try {
    ...
    echo "\n🔧 Solution : Créer un customer avec email unique pour le tenant 63\n";
    
    // Générer un email unique pour le tenant 63
    $uniqueEmail = 'majorelle.tenant63@morada-management.com';
    
    // Étape 1: Créer un user avec email unique pour le tenant 63
    $stmt = $pdo->prepare("SELECT id FROM users WHERE email = ? AND tenant_id = ?");
    $stmt->execute([$uniqueEmail, 63]);
    ...
    if ($existingUser) {
        ...
        echo "📝 Création d'un user avec email unique pour le tenant 63\n";
        ...
        $result = $stmt->execute(['milca VODUNON', $uniqueEmail, '+22945665432', 63]);
    ...
    if ($existingCustomer) {
        ...
        $result = $stmt->execute(['milca VODUNON', $uniqueEmail, '+22945665432', $userIdForTenant63, 63]);
    ...
    if ($final) {
        ...
        if ($final['customer_name']) {
            ...
            echo "✅ Email unique utilisé: " . $final['customer_email'] . "\n";
            ...
            echo "\n💡 Note: Un email unique " . $final['customer_email'] . " a été créé pour éviter les conflits\n";
        } else {
